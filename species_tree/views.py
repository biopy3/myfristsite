...
def clustalx_save(request):
    ...
    if request.method == "POST":
        ...
        if user_input.is_valid():
            ...
            inputfile = request.FILES['inputfile']
            access_code = str(uuid.uuid1())
            record = clustalx_model.objects.create(user=user_name, inputfile=inputfile,
                                            access_code=access_code,email=email)
            infile_path = record.inputfile.path
            clustalx_for_align.delay(infile_path,email,user_name,access_code)
            return HttpResponse("Thank you,we will send email for you!")
        else :
            error_msg = user_input.errors
...
def clustalx_result_download(request,access_code):
    try:
        ...
        return HttpResponse("So sorry,Some errors hanppened!")
